...
function Allproducts() {
  ...
  const uniqueCategories = [...new Set(product.map((p) => p.category.toLowerCase()))]
  const uniquePrices = [...new Set(product.map((p) => p.price))]

  // Apply filters locally
  const filteredProducts = product
    .filter((obj) => obj.title.toLowerCase().includes(searchkey.toLowerCase()))
    .filter((obj) => filterType ? obj.category.toLowerCase() === filterType.toLowerCase() : true)
    .filter((obj) => filterPrice ? obj.price.toString() === filterPrice : true)

  return (
    <Layout>
      {/* Filter Inputs */}
      <div className="flex flex-col md:flex-row gap-4 px-5 py-4 md:py-6 items-center justify-center">
        <input
          type="text"
                    ...
                    <div className="p-5 border-t-2">
                      ...
                      <div className="flex justify-center">
                        <button
                          ...
                          className="focus:outline-none text-white bg-green-500 hover:bg-pink-700 focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-sm w-full py-2"
                        >
                          Add To Cart
                        </button>
                      </div>
